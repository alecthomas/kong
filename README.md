...
| `lastwins:"X"`       | Last-wins group for flags, restricted to the same command. When multiple flags in the group appear on the command line, the last occurrence wins; earlier occurrences of other group members are removed from the effective trace as if they had not been given, and those flags revert to their normal fallback behavior (zero value, default, envar or resolver). When combined with `required`, at least one of the group will be required.       |
